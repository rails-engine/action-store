...
class ActionStore::MixinTest < ActiveSupport::TestCase
  ...
  test ".create_action" do
    ...
    assert_equal 1, Action.where(action_type: "like", target_type: "Post").count
    assert_equal 1, post.like_by_user_actions.count
    ...
    Action.transaction do
      User.create_action("like", target: post, user: post.user)
      User.create_action("like", target: post, user: post.user, action_option: "aaa")
      User.create_action("like", target: post, user: post.user, action_option: "bbb")
    end
